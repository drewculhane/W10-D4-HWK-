...
-- 7. The average salary of all NFL players
        SELECT ROUND(AVG(salary)) FROM players ; 

-- 8. The names and positions of players with a salary above 10_000_000
      SELECT name, position FROM players WHERE salary > 10000000 ORDER BY salary DESC ; 
...
-- 11. The average salary for a DE in the nfl
        SELECT ROUND(AVG(salary)) FROM players WHERE position = 'DE' ; 
